...
if($_SESSION['ROLE']==1){
	$sql="select `leave`.*, students.name,students.id as eid from `leave`,students where `leave`.student_id=students.id order by `leave`.id desc";
}else{
	$eid=$_SESSION['USER_ID'];
	$sql="select `leave`.*, students.name ,students.id as eid from `leave`,students where `leave`.student_id='$eid' and `leave`.student_id=students.id order by `leave`.id desc";
}
